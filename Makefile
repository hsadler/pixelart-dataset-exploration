...
train-overfit:
	poetry run python -m cli train --overfit=True --batch-size=5 --num-epochs=75 --image-pixel-size=64

predict-overfit:
	poetry run python -m cli predict --image-pixel-size=64
...
train-small-image-overfit:
	poetry run python -m cli train --overfit=True --batch-size=5 --num-epochs=25 --image-pixel-size=32

predict-small-image-overfit:
	poetry run python -m cli predict --image-pixel-size=32
...
train:
	poetry run python -m cli train --batch-size=50 --num-epochs=400 --image-pixel-size=64
